...
def compute_low_funds_and_email(
    payload: LowFundsEmailRequest, current_user=Depends(get_current_user)
):
    data = find_low_funds_envelopes(
        [env.dict() for env in payload.envelopes],
        absolute_threshold=payload.absolute_threshold,
    ...
    body = (
        "Low funds detected:\n"
        + "\n".join(f"{env['name']}: {env['reason']}" for env in data)
        if data
        else "No low funds detected."
    )

    send_email_notification(
        to_email=payload.to_email, subject=payload.subject, body=body
    )
